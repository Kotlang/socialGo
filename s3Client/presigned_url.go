...
func GetPresignedUrlForPosts(tenant, userId, extension string) (string, string) {
	...
	imagePath := fmt.Sprintf("social/%s/%s/%d/%d-image.%s", tenant, userId, time.Now().Year(), time.Now().Unix(), extension)
